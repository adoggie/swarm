...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        [_gregorian setTimeZone:[NSTimeZone timeZoneWithAbbreviation:@"GMT"]];
        _borderWidth                = 4;
        _originY                    = _dayWidth;
...
-(void)setCalendarDate:(NSDate *)calendarDate
{
    ...
    NSDateComponents *components = [_gregorian components:_dayInfoUnits fromDate:calendarDate];
    components.hour         = 0;
    components.minute       = 0;
    components.second       = 0;
...
-(IBAction)tappedDate:(UIButton *)sender
{
    ...
    if(componentsDateSel.day != sender.tag || componentsDateSel.month != components.month || componentsDateSel.year != components.year)
    {
        ...
        componentsDateSel.year      = components.year;
            ...
            if (![self canSwipeToDate:[_gregorian dateFromComponents:componentsDateSel]])
            {
                [self performViewNoSwipeAnimation];
                return;
            }
...
- (void)drawRect:(CGRect)rect
{
    ...
    NSInteger weekdayBeginning      = [comps weekday];  // Starts at Sunday on sat
    weekdayBeginning -=1;
