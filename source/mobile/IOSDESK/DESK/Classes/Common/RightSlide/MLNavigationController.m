...
- (void)viewDidLoad
{
    ...
    UIPanGestureRecognizer *_recognizer = [[[UIPanGestureRecognizer alloc]initWithTarget:self action:@selector(paningGestureReceive:)]autorelease];
...
- (UIViewController *)popViewControllerAnimated:(BOOL)animated
{
    ...
    /*    change by liyuchang 2013 12 10  for memucontrol  */
    if ([self.screenShotsList count]==1||[self.screenShotsList count]==0) {
        self.backgroundView.hidden=YES;
        if (self.screenShotsList==0) {
            self.backgroundView.hidden=YES;
            [self.backgroundView removeFromSuperview];
            [_backgroundView release];_backgroundView=nil;
        }
...
-(BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch
{
    ...
    DLog(@"MLNav pan gesture receive");
    return YES;
}
