...
public class DeskDaoImpl implements DeskDao {
	...
	public String getDeskDataOAuth(String token,String secret,String instanceUrl,String apTableName,int page,int perPage) throws Exception{
		return getDeskDataOAuth(token, secret, instanceUrl, apTableName, null,page,perPage);
	}
	public String getDeskDataOAuth(String token,String secret,String instanceUrl,String apTableName,String start,int page,int perPage) throws Exception{
		...
		if(start!=null){
			/**只有指定的表可以做增量备份...*/
			if("/api/v2/cases".equals(apTableName)){
				apiUrl = apiUrl + "/search?since_updated_at="+start;
				apiUrl = apiUrl +"&page="+page+"&per_page="+perPage;
			}else{
				apiUrl = apiUrl +"?page="+page+"&per_page="+perPage;
			}
		}else{
		...
		System.out.println("成功抓取数据apTableName:"+res);
		in.close();
		return res.toString();
	}
