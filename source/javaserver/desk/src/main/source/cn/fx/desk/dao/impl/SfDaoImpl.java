...
public class SfDaoImpl implements SfDao{
	...
	public int getSfObjcetCountByTime(String instanceURL ,String accessToken,String apTableName,String start,String end)throws Exception{
		...
		String data = GeneralMethod.requestHeaderSfAPI(queryUrl,accessToken);
		JSONObject datajson = JSONObject.fromObject(data);
		if(datajson.containsKey("totalSize")){
			return datajson.getInt("totalSize");
		}
		return 0;
	}
	public String getSalesforceObjectByTime(String instanceURL ,String accessToken,String apTableName,String start,String end,int page,int perPage)throws Exception{
		//1、获取指定apTableName的字段属性，拼成sql==>需要提到分页前面取出,无需每次都调用
		...
		sql.append("+LIMIT+"+perPage);
		sql.append("+OFFSET+"+(page-1)*perPage);
		//2、获取对象信息
		String queryUrl = instanceURL + SfConstant.API_QUERY + sql.toString();
