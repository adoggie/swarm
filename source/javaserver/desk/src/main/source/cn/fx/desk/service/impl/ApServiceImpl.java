...
public class ApServiceImpl implements ApService {
	...
	public OrgUserAppConfig getAppConfig(int uid,int appId) throws Exception{
		//cache++
		Map map = new HashMap();
		map.put("userId", uid);
		map.put("appId", appId);
		...
		//添加检验token是否有效的方法...
		System.out.println("ouac.getAppRefreshToken():"+ouac.getAppRefreshToken());
		if(ouac!=null){
				...
				if(jsonObj!=null&&!jsonObj.isNullObject()){
					...
					try {
						...
						if(FxConstant.BIZ_MODEL_SATIS == bizModel){
							...
							//sfService.getSfOpportunity(batchId, userId);//1.0.0
