...
from sw2us.desert import misc
from sw2us.desert import message
from sw2us import desert
...
class BaseType:
	...
	@staticmethod
	def getDefaultDB():
		db = desert.app.BaseAppServer.instance().getNoSQLDB()
		if not db:
			db = _DATABASE
		return db
...
class PostFileRel(BaseType):
	"""
		投递文件信息记录
	"""
	NAME = 'postfile_rel'
	def __init__(self):
		BaseType.__init__(self,PostFileRel.NAME)
		self.sender_id = None		#发送者ID
		self.target_id = None		#接收者id
		self.file_id = None			#文件存储编号
		self.issue_time = None		#发送时间
		self.expire = None  		#接收失效时间
		self.confirm_time = None	#接收确认时间
		self.extra = None
		self.file_size = 0
		self.file_type = None
		self.file_degist = None
		self.file_name = None

# class UploadFileRel(PostFileRel):
# 	NAME = 'uploadfile_rel'
# 	def __init__(self):
# 		PostFileRel.__init__(self)
# 		BaseType.__init__(self,UploadFileRel.NAME)
...
class Notification(BaseType):
	...
	def __init__(self,sender_id=0,target_id=0,type=0,sender_type=0,skipconfirm=False):
		...
		self.p3 = None
		self.p4 = None
		self.issue_time = misc.currentTimestamp64()
...
class SendMessage(BaseType):
	...
	def __init__(self):
		...
		self.issue_time = misc.currentTimestamp64()          #邀请发起时间
		self.confirm_time = None                #确认时间
		self.confirm_result = message.SendMessagetatus.UNACKED            	#0-未确认;  1 -发送已确认
